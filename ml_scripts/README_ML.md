...
synthetic smoke-test set referenced further down -- `ml_scripts/train.py` can be pointed at
...
(`ml_scripts/policy_mlp.py::export_c_header()` / `ml_scripts/rule_policy.py::export_c_header()`) was
...
**`ml_scripts/rule_policy.py` exists** as a deterministic placeholder with the identical
...
`cfd_scripts/run_sweep.py`'s dual-rotor MRF zone sizing shrank proportionally with
...
`ml_scripts/eda_azimuth_sensitivity.py` can give a real answer -- the existing 700-case
...
`cfd_scripts/run_sweep.py`'s `co_rot` dataset fixed `rpm_upper = 900` for all 140 cases
...
`ml_scripts/dataset.py::load_co_rot()` raises by default if it's handed a single-rpm_upper
...
holds on the real re-run, `POLICY_OUTPUT_COLS` in `ml_scripts/policy_mlp.py` should drop from
